...
##              $> Remove-Device-For-Imaging.ps1
##   A2.     When prompted, enter an asset tag (without trailing X)
...
####
##   ALTERNATIVE USAGE:
##   B1.     Run script with an asset tag
##              $> Remove-Device-For-Imaging.ps1 0951021
...
## Site name for Configuration Manager
$CMSiteName = '__CHANGE_ME__'
$CMModuleLocation = 'C:\Program Files (x86)\ConfigMgr Console\bin\ConfigurationManager.psd1'
...
Import-Module $CMModuleLocation -ErrorVariable SCCMModuleGetError
...
New-PSDrive -Name "P01" -PSProvider "CMSite" -Root $CMSiteName -Description "Primary site" -ErrorAction SilentlyContinue -ErrorVariable PSDriveGetError
if ($PSDriveGetError) {
